...
- `ChatakGUI/record.py`: Headless recording runner that verifies mount readiness, tracks recording status, launches arecord with correct parameters, and logs operational events for unattended field recording sessions.
...
- `ChatakGUI/wrapper_arecord.py`: Computes arecord parameters from z_odas configuration, aligns capture timing with ODAS startup, and records raw chunks with consistent metadata for synchronized downstream processing.
- `ChatakGUI/zodas_to_wav.py`: Parses z_odas binary outputs, applies gain, exports channelized WAV files, and generates plotting diagnostics for channel behavior and timestamp drift during localization experiments.
